...
#  To check or get the process ID of the script:  
...
# System built-in notify-send command does not work with root user,
# But this does (https://stackoverflow.com/a/49533938/20888768)
...
case "$1" in
    -d|--daemon)
        ...
        notify-send "Xyce compilation and installation proccesses running in background with root process ID: $!"
        exit 0
        ;;
    *)
        ;;
...
function notify-error() {
    ...
    notify-send "Xyce compilation and installation aborted due to errors"
    exit 0
}
